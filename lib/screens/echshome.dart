...
import '../Models/Echs_model.dart';

class Echs1 extends StatefulWidget {
  const Echs1({Key? key}) : super(key: key);

  @override
  State<Echs1> createState() => _Echs1State();
}

class _Echs1State extends State<Echs1> {
  List<EchsModel> data = [];
  ...
  Future<void> getData() async {
    ...
    print(
        "https://iafpensioners.gov.in/ords/dav_portal/ECHS/ECHS/${cat}");
    final response = await http.get(Uri.parse(
        'https://iafpensioners.gov.in/ords/dav_portal/ECHS/ECHS/${cat}'));
    if (response.statusCode == 200) {
      var responseBody = jsonDecode(response.body);
      data = (responseBody["items"] as List)
          .map((data) => EchsModel.fromJson(data))
          .toList();
      setState(() {});
    } else {
